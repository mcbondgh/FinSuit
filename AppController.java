...
//            throw new RuntimeException(e);}
    ...
    public void checkUserAccess() {
        List<Node> menuItems = List.of(dashboardBox, financeBox, messageBox, customerBox, accountBox, transactionBox, loanBox, resourceBox, reportBox, settingsBox);
        sidebarPane.getChildren().removeAll(menuItems);
        int ROLE_ID = getRoleIdByName(AppController.rolePlaceholder);
        ObservableList<PermissionsEntity> permissionData = getModuleControlList(ROLE_ID);

        for (PermissionsEntity permissions: permissionData) {
            String moduleName = permissions.getModule_name();
            boolean isAllowed = permissions.isAllowModule();

            if(moduleName.equals("DASHBOARD") && isAllowed) {
                sidebarPane.getChildren().add(dashboardBox);
            }
            if (moduleName.equals("FINANCE") && isAllowed) {
                sidebarPane.getChildren().add(financeBox);
            }
            if (moduleName.equals("MESSAGE BOX") && isAllowed) {
                sidebarPane.getChildren().add(messageBox);
            }
            if (moduleName.equals("CUSTOMER ACCOUNT" ) && isAllowed) {
                sidebarPane.getChildren().add(customerBox);
            }
            if (moduleName.equals("TRANSACTIONS") && isAllowed) {
                sidebarPane.getChildren().add(transactionBox);
            }
            if (moduleName.equals("LOAN MANAGER") && isAllowed) {
                sidebarPane.getChildren().add(loanBox);
            }
            if (moduleName.equals("HUMAN RESOURCE") && isAllowed) {
                sidebarPane.getChildren().add(resourceBox);
            }
            if (moduleName.equals("REPORTS") && isAllowed) {
                sidebarPane.getChildren().add(reportBox);
            }
            if (moduleName.equals("SETTINGS") && isAllowed) {
                sidebarPane.getChildren().add(settingsBox);
            }
        }
    }
    ...
    private void navigationHandler() {
        ...
        customersButton.setOnAction(action -> {
            AccountsController.pageTitlePlaceHolder = customersButton.getText();
            try {
