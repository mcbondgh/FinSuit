...
public class SettingsController extends SettingModel implements Initializable {
    ...
    @FXML
    private TextField senderIdField, senderMailField;
    @FXML
    private MFXButton updateButton, uploadButton, updateSenderIdButton;
    @FXML
    private Pane systemInfoPane, apiPane, warningPane;
    @FXML
    private ImageView logoViewer;
    @FXML
    private Label imageName, apiKeyField;
    @FXML
    private CheckBox modifyButton;
    @FXML
    private ComboBox<String> templateSelector, operationSelector;
    @FXML
    private PasswordField accountPasswordField, passwordField;
    @FXML
    private TextField loanPercentageField;
    @FXML
    private Label percentageIndicator, taxIndicator;
    @FXML
    private ListView<String> templateListView;
    @FXML
    private TextArea messageBodyField;
    @FXML
    private TextField messageTitleField, withdrawalRateField;
    @FXML
    private MFXButton saveTemplateButton, cancelTemplateButton, saveOperationButton;
    ...
    boolean isNameFieldEmpty() {
        return institutionNameField.getText().isEmpty();
    }

    boolean isNumberFieldEmpty() {
    ...
    boolean isDigitalFieldEmpty() {
        return digitalAddField.getText().isEmpty();
    }

    boolean isSenderIdEmpty() {
        return senderIdField.getText().isEmpty();
    }

    boolean isAccountFieldEmpty() {
        return accountPasswordField.getText().isEmpty();
    }

    boolean isSenderMailFieldEmpty() {
        return senderMailField.getText().isEmpty();
    }
    ...
    boolean isModifyButtonChecked() {
        return modifyButton.isSelected();
    }

    boolean isTitleFieldEmpty() {
        return messageTitleField.getText().isEmpty();
    }

    boolean isMessageFieldEmpty() {
        return messageBodyField.getText().isEmpty();
    }
...
//              String getImageSource = ImageReadWriter.displayImage(item.getLogo());
    ...
    @FXML
    void saveOperationButtonClicked() {
    ...
    boolean roleSelectorEmpty() {
        return roleSelector.getValue().isEmpty();
    }

    @FXML
    private ComboBox<String> roleSelector;
    @FXML
    private MFXButton savePermissionButton;
    ...
    @FXML
    private CheckBox dashboardViewSelector, financeViewSelector, transactionViewSelector, customersViewSelector;
    @FXML
    private CheckBox reportViewSelector, resourceViewSelector, messageViewSelector, settingsViewSelector, loansViewSelector;

    @FXML
    private TableColumn<PermissionsEntity, Integer> accessIndexColumn;
    @FXML
    private TableColumn<PermissionsEntity, String> accessNameColumn;
    @FXML
    private TableColumn<PermissionsEntity, String> accessDescriptionColumn;
    @FXML
    private TableColumn<PermissionsEntity, MFXCheckbox> accessButtonColumn;
    ...
    void getModuleStatusByRoleName() {
        ...
        for (PermissionsEntity data : moduleData) {

           switch (data.getModule_name()) {
               case "SETTINGS" -> settingsViewSelector.setSelected(data.isAllowModule());
               case "REPORTS" -> reportViewSelector.setSelected(data.isAllowModule());
               case "HUMAN RESOURCE" -> resourceViewSelector.setSelected(data.isAllowModule());
               case "LOAN MANAGER" -> loansViewSelector.setSelected(data.isAllowModule());
               case "TRANSACTIONS" -> transactionViewSelector.setSelected(data.isAllowModule());
               case "CUSTOMER ACCOUNT" -> customersViewSelector.setSelected(data.isAllowModule());
               case "MESSAGE BOX" -> messageViewSelector.setSelected(data.isAllowModule());
               case "FINANCE" -> financeViewSelector.setSelected(data.isAllowModule());
               case "DASHBOARD" -> dashboardViewSelector.setSelected(data.isAllowModule());
               default -> {
                  settingsViewSelector.setSelected(false);
                  reportViewSelector.setSelected(false);
                  resourceViewSelector.setSelected(false);
                  loansViewSelector.setSelected(false);
                  transactionViewSelector.setSelected(false);
                  customersViewSelector.setSelected(false);
                  messageViewSelector.setSelected(false);
                  financeViewSelector.setSelected(false);
                  dashboardViewSelector.setSelected(false);
               }
           }
        }
    }
    ...
    int getAccessControlSelectedPermissions() {
        ...
        List<CheckBox> viewsList = List.of(dashboardViewSelector, financeViewSelector, messageViewSelector, customersViewSelector, transactionViewSelector,
                loansViewSelector, resourceViewSelector, reportViewSelector, settingsViewSelector);

        for (var viewData : viewsList) {
            String name = viewData.getAccessibleText();
            boolean status = viewData.isSelected();
...
//            String name = getAppModules().get(i).getAlias();
//            permissions.setModule_name(name);
//            permissions.setAllowModule(status);
//            permissions.setRole_id(ROLE_ID);
//            if (countDuplicateKeysInModulesTable(name, ROLE_ID) > 0) {
